...
public class Additioner extends Operater implements Callable<Void> {
    ...
    public Void call() {
        if (debug) {
            System.out.println("generic.Additioner thread " + String.valueOf(linesModuloClass) + " started!");
        }
        if (a.length == 0) {
            return null;
        }
        ...
        if (debug) {
            System.out.println("generic.Additioner thread " + String.valueOf(linesModuloClass) + " ended!");
        }
        return null;
    }
}